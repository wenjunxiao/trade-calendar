...
function VirtualCalendar(config, holidayFinder) {
  ...
  this.name = this._config.name || 'virtual';
  this.reload({});
}
...
VirtualCalendar.prototype.reload = function (config, holidayFinder) {
  ...
  this._oneDay = vc.oneDay || 24 * 60 * 60 * 1000;
  this._timeRatio = this._oneDay / (24 * 60 * 60 * 1000);
  let strategy = this.strategy = vc.strategy || Strategy.STANDARD;
  this._virtualBase = vc.standard && moment.tz(vc.standard, this.timezoneName).valueOf() || this.startTime;
...
VirtualCalendar.prototype.now = function () {
  const time = Date.now();
...
VirtualCalendar.prototype.logArgsRealStamp = debug.enabled ? function (args) {
  return _.reduce(args, (r, v, k) => {
    if (v.getTime) {
      r[k] = v.format && v.format() || v._date && v._date.format && v._date.format() || moment(v._date || v).format();
    } else {
      r[k] = v;
    }
    return r;
...
VirtualCalendar.prototype.logArgsStamp = debug.enabled ? function (args) {
  return _.reduce(args, (r, v, k) => {
    if (v.valueOf) {
      r[k] = moment(v.valueOf()).format('YYYY-MM-DD HH:mm:ss.SSS');
    } else {
      r[k] = v;
    }
    return r;
...
VirtualCalendar.prototype.logRealStamp = debug.enabled ? function (stamp) {
  return moment(stamp).format();
} : ()=> {
};

VirtualCalendar.prototype.logStamp = debug.enabled ? function (stamp) {
  return moment.tz(stamp, this.timezoneName).format();
} : ()=> {
};
...
VirtualCalendar.prototype.wrap = function () {
  ...
  function _wrapDate(arg) {
    const fv = moment.tz(self.timestamp(arg.valueOf()), self.timezoneName).format(fmt);
    arg.setTime(moment(fv).valueOf());
    return arg;
  }

  function _wrapMoment(arg) {
    const fv = moment.tz(self.timestamp(arg.valueOf()), self.timezoneName).format(fmt);
    return new Date(moment(fv).valueOf());
  }

  function _wrapArgs() {
    let args = [].slice.call(arguments).map((arg) => {
      if (arg instanceof Date) {
        return _wrapDate(arg);
      } else if (arg instanceof moment) {
        return _wrapMoment(arg);
      } else if (arg._date) {
        if (arg._date instanceof Date) {
          return _wrapDate(arg._date);
        } else if (arg._date instanceof moment) {
          return _wrapMoment(arg._date);
        }
      }
      return arg;
    });
    if (args.length === 0) {
      const fv = moment.tz(self.currentStamp(), self.timezoneName).format(fmt);
      args.push(new Date(moment(fv).valueOf()));
    }
    return args;
  }
  ...
  function _wrap(fn) {
    ...
    if (fn.constructor && fn.constructor.name === 'GeneratorFunction') {
      _fn = function *() {
        ...
        let args = _wrapArgs.apply(null, _arguments); // 真实时间转化成日历时间
        let date = yield fn.apply(this, args);
        let original = moment.tz(moment(date).format(fmt), self.timezoneName).valueOf();
        let changed = self.realStamp(original);
        debug('change timestamp: %j %j %s %s', self.logArgsRealStamp(_arguments), self.logArgsStamp(args), self.logStamp(original), self.logRealStamp(changed));
      ...
      _fn = function () {
        let _arguments = arguments;
        let args = _wrapArgs.apply(null, _arguments); // 真实时间转化成日历时间
        let date = fn.apply(this, args); // 根据日历时间,计算出下一次符合的时间
        if (typeof date.then === 'function') {
          return new Promise((resolve, reject) => {
            let _resolve = (data) => {
              let original = moment.tz(moment(data).format(fmt), self.timezoneName).valueOf();
              let changed = self.realStamp(original);
              debug('change timestamp: %j %j %s %s', self.logArgsRealStamp(_arguments), self.logArgsStamp(args), self.logStamp(original), self.logRealStamp(changed));
...
VirtualCalendar.prototype.moment = function () {
  ...
  const v = moment.tz((!arguments[0] || typeof arguments[0] === 'number') ? m.valueOf() : m.format('YYYY-MM-DD HH:mm:ss.SSS'), this.timezoneName);
  return moment.tz(this.timestamp(v.valueOf()), this.timezoneName);
};
...
VirtualCalendar.prototype.setTimeout = function (code, delay, ...args) {
  return this._setTimeout(code, delay * this._timeRatio, ...args);
};
