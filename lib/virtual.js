...
VirtualCalendar.prototype.wrap = function () {
  ...
  function _wrap(fn) {
    ...
    if (fn.constructor && fn.constructor.name === 'GeneratorFunction') {
      _fn = async function () {
        let date = await fn.apply(this, arguments);
...
VirtualCalendar.prototype.moment = function () {
  ...
  const v = moment.tz(typeof arguments[0] === 'number' ? m.valueOf() : m.format('YYYY-MM-DD HH:mm:ss.SSS'), this.timezoneName);
...
VirtualCalendar.prototype.setTimeout = function (code, delay) {
  return setTimeout(code, delay * this._timeRatio);
};
