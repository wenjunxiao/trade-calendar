 ...
 */
TradeCalendar.prototype.findHolidays = function*(start, end) {
  let holidays = yield this._holidayFinder(start, end);
  if (!holidays) return [];
  holidays.sort((a, b) => {
 ...
 * @returns {TimeInfo}
 */
TradeCalendar.prototype.getTimeInfo = function*(date, direction) {
  let timeInfo = yield this.timeInfo(date || this.today(), direction);
  return {
    tradeDate: timeInfo.tradeDate,
 ...
 */
TradeCalendar.prototype.timeInfo = function*(date, direction) {
  ...
  for (date = momentFactory(date);; date.add(direction, 'day')) {
    ...
    if (days > 0 && days < 6) { // 排除周末
      ...
      // 查找这段时间内的节假日
      let holidays = yield this.findHolidays(start, end);
 ...
 */
TradeCalendar.prototype.currentTime = function() {
